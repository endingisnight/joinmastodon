 ...
 * @param {AppCardProps} app 
 * @param {string} activeCategory
 * @returns {string}
 */
function getAppUrl(app, activeCategory, intl) {
  ...
  switch (activeCategory) {
    ...
    case `flatpak`:
      return `https://${app.flatpak.replace(`/`, `/app/`)}`;
    case `web`:
      return app.web;
    case `source`:
      return app.source;
 ...
 */
export const AppCard = (app: AppCardProps, activeCategory: string) => {
  const {
    name,
    icon,
    url,
  ...
  return (
    ...
    >
      <a className="h-[3.5rem] w-[3.5rem] flex-shrink-0 overflow-hidden rounded-sm" href={getAppUrl(app, activeCategory)}>
      ...
      <div className="flex flex-auto flex-col">
        ...
        <h3 className="b1 !font-700 flex flex-auto items-center !leading-[1] rtl:text-right">
          <a href={getAppUrl(app, activeCategory)} dir="ltr">{name}</a>
        </h3>
      </div>
    </div>
  )
